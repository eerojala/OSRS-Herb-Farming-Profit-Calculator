...
const getSeedAndCleanHerbData = async (name) => {
    ...
    return response.data.items.filter(item => seedIds.includes(item.id) || cleanHerbIds.includes(item.id))
}
...
const splitSeedAndCleanHerbData = (seedAndCleanHerbData) => {
    const seedData = new Array(14)
    const cleanHerbData = new Array(14)
...
const convertPriceToNumber = (price) => {
    ...
    } else { // Prices under 1000 are presented as normal integers
        return Number(price)
    }
...
const getDataFromAPI = async () => {
    try {
        ...
        console.log('Successfully fetched data from the OSRS Grand Exchange API at ', new Date())
        return convertJSONToItemObjects(seedData, cleanHerbData, grimyHerbData)
    } catch (exception) {
